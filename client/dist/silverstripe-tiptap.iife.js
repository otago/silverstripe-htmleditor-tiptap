...
3. "-" cannot repeat`);
  ...
  function computeMap(table) {
    ...
    let problems = null;
    const colWidths = [];
    for (let i2 = 0, e = width * height; i2 < e; i2++) map[i2] = 0;
  ...
  var CellSelection = class _CellSelection extends Selection {
    ...
    constructor($anchorCell, $headCell = $anchorCell) {
      ...
      const ranges = cells.map((pos) => {
        ...
        return new SelectionRange(
          doc2.resolve(from),
          doc2.resolve(from + cell.content.size)
        );
      });
    ...
    static colSelection($anchorCell, $headCell = $anchorCell) {
      ...
      if (anchorRect.top <= headRect.top) {
        if (anchorRect.top > 0)
          $anchorCell = doc2.resolve(tableStart + map.map[anchorRect.left]);
        ...
        if (headRect.top > 0)
          $headCell = doc2.resolve(tableStart + map.map[headRect.left]);
    ...
    static rowSelection($anchorCell, $headCell = $anchorCell) {
      ...
      if (anchorRect.left <= headRect.left) {
        ...
        if (headRect.left > 0)
          $headCell = doc2.resolve(tableStart + map.map[headRect.top * map.width]);
  ...
  function addColumn(tr2, { map, tableStart, table }, col) {
  ...
  function removeColumn(tr2, { map, table, tableStart }, col) {
  ...
  function addRow(tr2, { map, tableStart, table }, row) {
    var _a;
  ...
  function removeRow(tr2, { map, table, tableStart }, row) {
    let rowPos = 0;
    for (let i2 = 0; i2 < row; i2++) rowPos += table.child(i2).nodeSize;
  ...
  function fitSlice(nodeType, slice) {
    ...
    const tr2 = new Transform(node).replace(0, node.content.size, slice);
    return tr2.doc;
  }
  ...
  function growTable(tr2, map, table, start, width, height, mapFrom) {
  ...
  function isolateHorizontal(tr2, map, table, start, left, right, top, mapFrom) {
    ...
    for (let col = left; col < right; col++) {
      ...
      if (map.map[index - map.width] == pos) {
        ...
        tr2.insert(
          tr2.mapping.slice(mapFrom).map(map.positionAt(top, cellLeft, table)),
  ...
  function isolateVertical(tr2, map, table, start, top, bottom, left, mapFrom) {
  ...
  function insertCells(state, dispatch, tableStart, rect, cells) {
    ...
    if (growTable(tr2, map, table, tableStart, right, bottom, mapFrom)) recomp();
    if (isolateHorizontal(tr2, map, table, tableStart, left, right, top, mapFrom))
      recomp();
    if (isolateHorizontal(tr2, map, table, tableStart, left, right, bottom, mapFrom))
      recomp();
    if (isolateVertical(tr2, map, table, tableStart, top, bottom, left, mapFrom))
      recomp();
    if (isolateVertical(tr2, map, table, tableStart, top, bottom, right, mapFrom))
      recomp();
    for (let row = top; row < bottom; row++) {
      const from = map.positionAt(row, left, table), to = map.positionAt(row, right, table);
      tr2.replace(
        tr2.mapping.slice(mapFrom).map(from + tableStart),
    ...
    tr2.setSelection(
      new CellSelection(
        tr2.doc.resolve(tableStart + map.positionAt(top, left, table)),
        tr2.doc.resolve(tableStart + map.positionAt(bottom - 1, right - 1, table))
      )
    );
    dispatch(tr2);
  }
    ...
    $.entwine("ss", function($2) {
      $2("textarea.htmleditor").entwine({
        ...
        getToolbarItemsConfig: function(config) {
          ...
          return {
            ...
            "dropdown": {
              ...
              extension: "custom",
              options: []
              // Will be populated dynamically from config.extensions
            },
            "newline": {
              type: "newline"
            },
        ...
        createToolbar: function(wrapper, editor, config) {
          const toolbar = $2(`<div class="${CONSTANTS.CSS_CLASSES.TOOLBAR}"></div>`);
          ...
          toolbarLayout.forEach((item) => {
            ...
            if (!itemConfig) {
              ...
              const itemConfig2 = {
                ...
                extension: itemName,
                title: buttontitle,
                // Full text with shortcuts for tooltip
                type: "button",
                buttontext: ""
                // Just the title part for button text
              };
              const button = this.createToolbarButton(itemConfig2, editor);
              toolbar.append(button);
              return;
            }
            if (itemConfig.type === "separator") {
              const separator = $2(`<div class="${CONSTANTS.CSS_CLASSES.SEPARATOR}"></div>`);
              toolbar.append(separator);
              return;
            }
            if (itemConfig.type === "newline") {
              const lineBreak = $2(`<div class="${CONSTANTS.CSS_CLASSES.NEWLINE}"></div>`);
            ...
            const hasRequiredExtension = itemConfig.extension === "custom" || requiredExtensions.some(
              (extName) => editor.extensionManager.extensions.find((ext) => ext.name === extName)
            );
        ...
        addDropdownToggle: function(button, dropdownMenu) {
          button.on("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            $2(`.${CONSTANTS.CSS_CLASSES.TOOLTIP}.${CONSTANTS.CSS_CLASSES.SHOW}`).removeClass(CONSTANTS.CSS_CLASSES.SHOW);
          ...
          $2(document).on("click", () => {
            ...
            $2(".tiptap-tooltip.show").removeClass("show");
          });
        },
        // Create generic dropdown for grouped objects
        createGenericDropdown: function(item, dropdownConfig, editor, config) {
          const dropdown = $2(`<div class="${CONSTANTS.CSS_CLASSES.DROPDOWN}"></div>`);
          const button = $2(`<button type="button" data-action="${item.title}" class="${item.icon}"></button>`);
          const dropdownMenu = $2(`<div class="${CONSTANTS.CSS_CLASSES.DROPDOWN_MENU}"></div>`);
        ...
        createToolbarButton: function(itemConfig, editor) {
          const button = $2(`<button type="button" data-action="${itemConfig.action}">${itemConfig.buttontext}</button>`);
          const self = this;
        ...
        createToolbarDropdown: function(itemConfig, editor, toolbarConfig) {
          const dropdown = $2(`<div class="${CONSTANTS.CSS_CLASSES.DROPDOWN}"></div>`);
          const button = $2(`<button type="button" data-action="${itemConfig.action}"></button>`);
          const dropdownMenu = $2(`<div class="${CONSTANTS.CSS_CLASSES.DROPDOWN_MENU}"></div>`);
          const self = this;
          ...
          if (itemConfig.options) {
            itemConfig.options.forEach((option) => {
              ...
              if (itemConfig.action === "heading") {
                ...
                optionBtn = $2(`<button type="button" data-level="${option.level || "paragraph"}">${parts.title}</button>`);
                ...
                const parts = this.parseTooltipText(option.text);
                optionBtn = $2(`<button type="button" data-list-type="${option.type}">${parts.title}</button>`);
                this.addTooltip(optionBtn, option.text);
                optionBtn.on("click", (e) => {
                  e.preventDefault();
                  e.stopPropagation();
                ...
                if (option.isGroup) {
                  const groupItem = $2(`<div class="${CONSTANTS.CSS_CLASSES.DROPDOWN_GROUP_ITEM}">
                  <span class="${CONSTANTS.CSS_CLASSES.GROUP_TEXT}">${option.text}</span>
                  <span class="${CONSTANTS.CSS_CLASSES.GROUP_ARROW}">›</span>
                </div>`);
                  const submenu = $2(`<div class="${CONSTANTS.CSS_CLASSES.SUBMENU}"></div>`);
                  if (option.children && option.children.length > 0) {
                    option.children.forEach((child) => {
                      if (child.isStyle) {
                        const parts = this.parseTooltipText(child.text);
                        const childBtn = $2(`<button type="button" data-style-class="${child.className}" class="${CONSTANTS.CSS_CLASSES.STYLE_OPTION}">${parts.title}</button>`);
                  ...
                  $2("body").append(submenu);
                  groupItem.data("submenu", submenu);
                  dropdownMenu.append(groupItem);
                  return;
                } else if (option.isStyle) {
                  const parts = this.parseTooltipText(option.text);
                  optionBtn = $2(`<button type="button" data-style-class="${option.className}" class="${CONSTANTS.CSS_CLASSES.STYLE_OPTION}">${parts.title}</button>`);
                  this.addTooltip(optionBtn, option.text);
                ...
                const parts = this.parseTooltipText(option.text);
                optionBtn = $2(`<button type="button" data-table-action="${option.action}">${parts.title}</button>`);
                this.addTooltip(optionBtn, option.text);
                optionBtn.on("click", (e) => {
                  e.preventDefault();
                  e.stopPropagation();
        ...
        updateToolbarButtonState: function(btn, action, editor, config) {
          ...
          switch (action) {
            ...
            case "fullscreen":
              if (screenfull$1.isEnabled) {
                btn.toggleClass("active", screenfull$1.isFullscreen);
        ...
        addTooltip: function(button, tooltipText) {
          ...
          const tooltip = $2(`<div class="${CONSTANTS.CSS_CLASSES.TOOLTIP}"></div>`);
          const tooltipTitle = $2('<div class="tooltip-title"></div>').text(parts.title);
          tooltip.append(tooltipTitle);
          if (parts.shortcut) {
            const tooltipShortcut = $2('<div class="tooltip-shortcut"></div>').text(parts.shortcut);
            tooltip.append(tooltipShortcut);
          }
          $2("body").append(tooltip);
          button.data("tooltip", tooltip);
          button.on("mouseenter focus", function(e) {
            self.showTooltip($2(this), tooltip);
          });
...
//# sourceMappingURL=silverstripe-tiptap.iife.js.map
